...
본 레포에도 `docker-compose.yml` 이 있지만 **공유 인프라용으로 띄우지 않는다.**
운영/개발 시 인프라는 `newtech-platform` 측에서 띄우고, 본 레포의 백엔드는
호스트의 5433/6379 에 붙는다. 둘 다 띄우면 컨테이너 이름·볼륨·포트가 충돌한다.
**예외**: newtech-platform 없이 단독 디버그 시에만 본 레포 compose 사용.
...
```bash
# 1) 인프라는 newtech-platform 에서
cd /Users/lmj/00_projects/newtech-platform && docker compose up -d postgres redis

# 2) 본 레포
cd /Users/lmj/00_projects/newtech_data/kb-estate-collector
source .venv/bin/activate
alembic upgrade head                                                 # 최초/모델 변경 시
uvicorn src.admin_api.main:app --host 0.0.0.0 --port 8000 --reload   # Admin API

# 3) Celery worker (별도 터미널, prefork)
...
INTEGRATION.md
docker-compose.yml    # 단독 디버그용 (운영 인프라는 newtech-platform 쪽)
```
...
### 마이그레이션
```bash
alembic revision --autogenerate -m "add foo column"   # autogenerate 결과 검토 필수
...
- **상황**: 워커 기동 후 `SIGSEGV` 빈발 → 좀비 task 누적 → 수집 멈춤
...
### 7. 인프라(postgres/redis)를 본 레포 docker-compose 로 띄우지 말 것
- **상황**: 개발 환경 셋업
- **하지 말 것**: `docker compose up -d postgres redis` 를 본 레포에서 실행 → newtech-platform 의 컨테이너와 이름·볼륨·포트 충돌.
- **할 것**: 인프라는 `newtech-platform` 측 compose 가 단일 진실(single source). 본 레포는 호스트의 5433/6379 에 연결. 단독 디버그가 필요할 때만 본 레포 compose 사용.
