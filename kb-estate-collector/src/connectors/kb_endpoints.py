...
# 실거래가 (매매/전세/월세 통합) — KB 웹사이트 실거래가 탭이 사용하는 엔드포인트.
# vlaDealDtailPriceInq 보다 데이터 풍부 (계약취소/등기일자/최고가여부 등 포함).
COMPLEX_PRESALE_PRICES = KBEndpoint(
    name="complex_presale_prices",
    base_url=KB_API_BASE,
    path="/land-price/price/complex/preSalePrices",
    method="GET",
    description=(
        "실거래가 통합 (매매/전세/월세). 파라미터: 단지기본일련번호, 면적일련번호, "
        "거래구분(0=전체/1=매매/2=전세/3=월세), 면적그룹여부, 현재페이지, 페이지갯수, 첫페이지갯수. "
        "응답: dataBody.data.dataList[].{계약년월일, 물건거래구분, 매매실거래금액(만원), "
        "전세실거래금액, 보증금액, 월세금액, 해당층수, 전용면적, 계약취소여부, 등기일자, "
        "역대최고가여부, 최근1년최고가여부 등}"
    ),
)
