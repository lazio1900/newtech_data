...
class ComplexFacility(Base):
    ...
    facility_type = Column(
        String(20),
        nullable=False,
        comment="대분류: school/subway/hospital/cctv/ev_charger/park/mart/bank",
    )
    sub_type = Column(
        String(40),
        nullable=True,
        comment="소분류: kindergarten/preschool/elementary/middle/high, line2 등",
    )

    # 시설 정보
    external_id = Column(String(80), nullable=True, comment="KB 내부 식별자 (학교식별자 등)")
    name = Column(
        String(500), nullable=False, comment="시설명 (다중 시설 파이프 합성 응답 대비 500자)"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "complex_id", "facility_type", "external_id", name="uq_facility_complex_type_extid"
        ),
